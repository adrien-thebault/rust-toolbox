...
///
/// The stream opens with a comment, so the response body's first byte is
/// written the moment the connection is accepted rather than when the first
/// real event arrives. A buffering proxy (nginx with `proxy_buffering on`, the
/// WSL2 localhost relay) holds the whole response, headers included, until that
/// first byte - so without it a client can sit in "connecting" for as long as
...
/// * `cfg` - Heartbeat interval. A stream without a heartbeat dies silently
///   behind a proxy.
...
{
    let hello = stream::once(async { Ok(Event::default().comment("connected")) });
    let stream = hello.chain(events.map(|event| {
